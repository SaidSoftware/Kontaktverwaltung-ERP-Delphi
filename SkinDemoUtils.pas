...
{$IFDEF EXPRESSBARS}
procedure CreateSkinsMenuItem(ABarmanager: TdxBarManager; ACaption: string = '');
{$ELSE}
procedure CreateSkinsMenuItem(AMenu: TMainMenu; ACaption: string = '');
{$ENDIF}
  ...
  begin
    ...
    begin
      if AMenuItem.Count = 0 then
        ABarItem := ABarManager.AddButton
      else
      begin
...
initialization
  dxUnitsLoader.AddUnit(SysInit.HInstance, 'SkinDemoUtils', RegisterAssistants, UnregisterAssistants);

finalization
  dxUnitsLoader.RemoveUnit(SysInit.HInstance, 'SkinDemoUtils', UnregisterAssistants);
